  ...
  return (
    <PairLayout>
      <Head>
        ...
        <title>{i18n._(t`Borrow ${pair?.asset?.symbol}-${pair?.collateral?.symbol}`)} | NEXUSSwap</title>
        <meta
          key="description"
          name="description"
          /* @ts-ignore TYPE NEEDS FIXING */
          content={`Borrow ${pair?.asset?.symbol} against ${pair?.collateral?.symbol} collateral on Kashi by NEXUSSwap`}
        />
        <meta
      ...
      right={
        <Card className="h-full p-4 bg-dark-900 xl:p-0">
          <div className="flex-col space-y-2">
            ...
            {/*@ts-ignore TYPE NEEDS FIXING*/}
            {pair && pair.oracle.name === 'NEXUSSwap' && (
              <>
                <div className="flex justify-between pt-3">
